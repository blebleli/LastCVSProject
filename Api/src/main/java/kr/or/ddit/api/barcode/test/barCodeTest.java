...
public class barCodeTest {
	...
	public static void main(String[] args) {
		...
		if (null == barcode_text) {
			System.err
					.println("Some barcode text must be specified with the -b option");
			Usage();
			return;
		}
		try {
			...
			if (null != label_outfile) {
				OutputStreamWriter pw = new OutputStreamWriter(
						new FileOutputStream(label_outfile));
	...
	public static void Usage() {
		...
		System.err
				.println("java -jar barbecue.jar -b <barcode text> -e <encoding> -o <outfile> {-E}");
		System.err.println("\tAvailable Encodings are:");
		System.err.println("\t\t \"ean128\"");
		System.err.println("\t-E: Write output as EPS.");
		System.err
				.println("\t-label <filename>: Write human readable (including check digit) text to the file.");
		System.err
				.println("\t-o <filename>: Write the barcode image to the file.");
		System.err
	...
	public static void outputSVG(Barcode barcode, OutputStream fos) {
		try {
			...
			SVGOutput svg_out = new SVGOutput(osw,
					DefaultEnvironment.DEFAULT_FONT, java.awt.Color.black,
					java.awt.Color.white, 1, "in");
