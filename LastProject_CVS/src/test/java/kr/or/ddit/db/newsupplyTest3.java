...
public class newsupplyTest3 { // 테스트입니다.
	...
	public void test() throws ParseException {
		...
		String supply_date_s = "2018/10/15 10:51";
	...
	public void test2() throws ParseException {
		...
		SupplyListVo sup = null;
		String supply_date_s = "2018/10/17 10:51";
		Date supply_date = new SimpleDateFormat("yyyy/MM/dd HH:mm")
				.parse(supply_date_s);
	...
	public void supplyListSum() throws ParseException{
		...
		String supply_date_s = "2018/10/16 10:51";
...
//				// 대전 65건
		...
		for(int i = 0; i < mem_id.length; i++){
			...
			List<StockListVo> stockList = template.selectList("test.getStockId", mem_id[i]); // 15일꺼. mem_id로 stock_id 찾기
//			stockListVo2 = template.selectOne("test.getStockIds", mem_id[i]); // 16일꺼. mem_id로 stock_id 찾기		
			...
			for (StockListVo vo : stockList) {
				...
				barcodeVo = new BarcodeVo();				
				barcodeVo.setBcd_id(bcd_id);      	 	 		    //바코드코드
				barcodeVo.setBcd_content("마감");  	   				 //내용			
//				String kind = ("888");
				barcodeVo.setBcd_kind("103"); 		      			//재고 : 100, 저장소 : 101, 수불 : 102 마감 :103
				...
				stockListVo.setBcd_id(barcodeVo.getBcd_id()); // 재고/마감코드
				stockListVo.setStcklist_amount(vo.getStcklist_amount()); // 수량
				stockListVo.setStcklist_exdate(vo.getStcklist_exdate()); // 유통기한
				stockListVo.setStck_date(vo.getStck_date()); // 날짜
				stockListVo.setStcklist_info("마감"); // 비고
				stockListVo.setStcklist_kind("999"); // 재고				
				stockListVo.setStock_id(stockVo.getStock_id()); // 재고 코드
				logger.debug("stockListVo.getStock_id() {}",stockListVo.getStock_id());
				stockListVo.setProd_id(vo.getProd_id()); // 제품코드
				stockListVo.setSplylist_id(vo.getSplylist_id()); // 제품리스트 코드				
